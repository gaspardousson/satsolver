...
let time path n_pb n_var =
    ...
    let t0 = Sys.time () in
    for i = 1 to n_pb do
        ignore (solver prob.(i-1) n_var)
    done;
    let t1 = Sys.time () in
    print_endline (string_of_int n_var ^ "-" ^ string_of_float ((t1-.t0)*.1000./.(float_of_int n_pb)))
;;
...
let () =
    ...
    |5 -> begin
            ...
            let n_var = int_of_string Sys.argv.(3) in
            let quantity = Sys.argv.(4) in
            match quantity with
                |"bool" -> bool path n_pb n_var
                |"time" -> time path n_pb n_var
