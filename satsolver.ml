...
let solveur_naif probleme =
    ...
    let rec main niv lit interpretation =
        ...
        let var = branchement n_var interpretation in
        if var > n_var
...
let solveur_quine probleme =
    ...
    let rec main niv lit interpretation =
        ...
        then begin
            let var = branchement n_var ~o:occurrence interpretation in
            if var > n_var
                then Sat
                else let lit = polarisation.(var) in
                    match main (niv+1) lit interpretation with
...
let solveur_cdcl probleme =
    ...
    let rec main niv lit graphe interpretation =
        ...
        then begin
            ...
            let backjump = analyse !position !graphe ~a:!activite !cnf temoignage in
            ...
            if !position >= !max_clauses
                then begin
                    position := nettoyage !moyenne !activite n_var n_clauses !max_clauses !graphe !cnf temoignage;
